...
float inflate_buffer = 5; // higher inflation buffer means that higher force is required to start the pumps
float deflate_buffer = 40; // higher deflation buffer means that a greater loss of force over the same time step is required to begin deflation
...
void control_pump(bool is_print) {
    ...
    if (pwm1 < pwm_min + inflate_buffer) { // pwm is too small to inflate the bladders to any noticeable amount
        digitalWrite(Pump_A1, LOW);
    ...
    if (pwm1 < prev_pwm1 - deflate_buffer) {
        ...
        delay(10);                  // set how much time the solenoid can be let open to determine how much air leaves the bladder
    ...
    if (pwm2 < pwm_min + inflate_buffer) { 
    ...
    if (pwm2 < prev_pwm2 - deflate_buffer) {
        ...
        delay(150);                 // set how much time the solenoid can be let open to determine how much air leaves the bladder
...
// control pump function for symposium demo with discrete PWMs instead of direct PWM mapping
// discrete PWM values have been chosen arbitrarily with some consideration on how much change
// in PWM makes a noticeable difference in the inflation with the pumps
void control_pump_demo(bool is_print) {
    // map FSR readings to pressure values as determined by FSR and pressure calibration
    pwm1 = map(analogRead(FSR1), 0, fsr_max1, pwm_min, pwm_max);
    pwm2 = map(analogRead(FSR2), 0, fsr_max2, pwm_min, pwm_max);
    ...
    //  pwm_min (54) + inflate_buffer (5)
    //  59 <= pwm < 75 ==> pwm = 60
    if (pwm1 >= pwm_min + inflate_buffer && pwm1 < 75) { pwm1 = 60; }
    if (pwm2 >= pwm_min + inflate_buffer && pwm2 < 75) { pwm2 = 60; }

    //  75 <= pwm < 90 ==> pwm = 75
    if (pwm1 >= 75 && pwm1 < 90) { pwm1 = 75; }
    if (pwm2 >= 75 && pwm2 < 90) { pwm2 = 75; }

    //  90 <= pwm < 105 ==> pwm = 90
    if (pwm1 >= 90 && pwm1 < 105) { pwm1 = 90; }
    if (pwm2 >= 90 && pwm2 < 105) { pwm2 = 90; }

    //  105 <= pwm < 115 ==> pwm = 105
    if (pwm1 >= 105 && pwm1 < 115) { pwm1 = 105; }
    if (pwm2 >= 105 && pwm2 < 115) { pwm2 = 105; }
...
void data_collection() {
    ...
    Serial.println("Inflate Bladder 2");
    for (byte i = 0; i < 3; i++) {
        inflate(0,     50);
        inflate(0,     70);
        inflate(0,     90);
...
// tests spatial differentiation and temporal resolution
// type 's' when pump 1 inflates and 'k' when pump 2 inflates
void reaction(bool debug) {
    // ensure the solenoids are in the closed position
    digitalWrite(Sol_A1, LOW);
    digitalWrite(Sol_B1, LOW);
    ...
    // while none of the sensors have been pressed, read from the sensors
    while(pump_select == 0) {
        if(debug) {
            Serial.print("FSR 1:\t");
            Serial.print(analogRead(FSR1));
        ...
        if(analogRead(FSR1) > 120) { pump_select = 1; }
        else if (analogRead(FSR2) > 120) { pump_select = 2; }
    }
    
    if(debug) {
        Serial.print("Pump Selected:\t");
        Serial.println(pump_select);
        delay(1000);
    ...
    if (pump_select == 1) {
        int long start_time = millis(); // start a timer

        // while the 's' has not been pressed, inflate bladder 1
        while(!is_pressed) {
            // inflate pumps
            digitalWrite(Sol_A1, LOW);
            analogWrite(Pump_A_EN, 80);
            digitalWrite(Pump_A1, HIGH);

            if(Serial.available() > 0) {
                // intake keyboard press
        ...
        // stop inflating pumps and open solenoids
        analogWrite(Pump_A_EN, 0);
        digitalWrite(Pump_A1, LOW);
        digitalWrite(Sol_A1, HIGH);
        delay(1000); // give time for the pumps to fully deflate
        digitalWrite(Sol_A1, LOW);
        ...
        // while the 'k' has not been pressed, inflate bladder 2
        while(!is_pressed) {
            // inflate pumps
            digitalWrite(Sol_B1, LOW);
            analogWrite(Pump_B_EN, 80);
            digitalWrite(Pump_B1, HIGH);
...
// Calls the reaction function many times
void test_reaction(int num_tests, bool debug) {
    for(byte i = 0; i < num_tests; i++) {
        Serial.print("Performing Test #");
        Serial.println(i);
        delay(100);
        ...
        Serial.println("Type 'n' to start next test or 'e' to exit");
        delay(100);

        while(!is_continue) {
            if(Serial.available() > 0) {
                char character = Serial.read();
                if(character == 'n') { is_continue = true; }
                else if(character == 'e') { 
                    Serial.println("Exiting...");
                    delay(1000);
                    exit(0);
